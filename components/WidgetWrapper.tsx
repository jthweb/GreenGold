...
const WidgetWrapper: React.FC<WidgetWrapperProps> = ({ title, children, explanation, explanationPrompt, onExplain, hasNoHeader = false }) => {
    const { t } = useLocalization();

    if (hasNoHeader) {
        return (
            <div className="bg-transparent h-full flex flex-col transition-transform duration-300">
                {children}
            </div>
        );
    }
