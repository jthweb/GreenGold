                ...
                <div className="absolute end-0 bottom-full mb-2 w-56 bg-white dark:bg-[#2a3831] rounded-lg shadow-xl z-20 ring-1 ring-black ring-opacity-5 animate-fade-in-fast">
                    <div className="py-1 max-h-72 overflow-y-auto">
                        ...
                        {showAll && otherLanguages.map(lang => (
                             <button
                                key={lang.code}
                                onClick={() => handleSelect(lang.code)}
                                className={`w-full text-left px-4 py-2 text-sm transition-colors animate-fade-in-fast ${language === lang.code ? 'font-semibold text-[#D4A22E]' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700/60'}`}
                            >
                                {lang.name}
                            </button>
                        ))}
                    ...
                    {!showAll && (
                        <div className="border-t border-slate-200 dark:border-slate-700/50 p-1">
                            <button onClick={() => setShowAll(true)} className="w-full text-center px-4 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:text-[#D4A22E] font-semibold rounded transition-colors">
                                Show all...
                            </button>
                        </div>
                    )}
